...
class Plans(unittest.TestCase):
 def setUp(self):
  ...
  self.actions={'browser':[], 'terminal':[], 'files':[]};self.labels={a:a.title() for a in self.actions}
  self.calls=[]
 def prepare(self,ids=('browser','terminal'),created=None):
  import time
  put(self.base/'command-plan.json',dict(token='a'*32,created=time.time() if created is None else created,actions=list(ids)),False)
  return 'plan:'+'a'*32
 def execute(self,token,available=lambda a:True,run=None):
  def default(a):self.calls.append(a);return {'text':'ok'}
 ...
 def test_failure_stops(self):
  def run(a):self.calls.append(a);return {'ok':a!='terminal'}
 ...
 def test_rewrite(self):
  self.assertEqual(plans.rewrite(['radio_random','cartoons_on','cartoons_mute']),['radio_random','cartoons_on_muted'])
